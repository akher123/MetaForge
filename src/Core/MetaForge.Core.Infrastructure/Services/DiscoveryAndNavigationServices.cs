...
public class EntityMetadataDiscoveryService : IEntityMetadataDiscoveryService
{
    ...
    public EntityMetadataDiscoveryService(IModuleDbContextResolver contextResolver, IUnitOfWork unitOfWork)
    {
    ...
    public IReadOnlyList<EntityMetadataDto> DiscoverAll() =>
        GetFeatureEntityTypes()
            .Select(MapEntity)
            .ToList();

    public EntityMetadataDto? Discover(string entityName) =>
        GetFeatureEntityTypes()
            .FirstOrDefault(t => string.Equals(t.ClrType.Name, entityName, StringComparison.Ordinal)) is { } entityType
    ...
    public async Task GenerateFormConfigurationAsync(string entityName, CancellationToken cancellationToken = default)
    {
        ...
        var entityClrType = GetFeatureEntityTypes()
        ...
        var module = new ForgeForm
        {
            ...
            GroupName = InferModuleGroupName(entityClrType?.Namespace),
    ...
    private IEnumerable<Microsoft.EntityFrameworkCore.Metadata.IEntityType> GetFeatureEntityTypes()
    {
        var seen = new HashSet<string>(StringComparer.Ordinal);

        foreach (var module in _contextResolver.EnabledModules)
        {
            var context = _contextResolver.ResolveForModule(module.Name);
            foreach (var entityType in context.Model.GetEntityTypes())
            {
                if (!FeatureDiscoveryConstants.IsFeatureEntityNamespace(entityType.ClrType.Namespace))
                    continue;

                if (seen.Add(entityType.ClrType.FullName ?? entityType.ClrType.Name))
                    yield return entityType;
            }
        }

        foreach (var entityType in _contextResolver.CoreContext.Model.GetEntityTypes())
        {
            if (!FeatureDiscoveryConstants.IsFeatureEntityNamespace(entityType.ClrType.Namespace))
                continue;
    ...
    private static string InferModuleGroupName(string? entityNamespace)
    {
        if (entityNamespace?.StartsWith("MetaForge.", StringComparison.Ordinal) == true
            && entityNamespace.Contains(".Domain.", StringComparison.Ordinal))
        {
            var rest = entityNamespace["MetaForge.".Length..];
