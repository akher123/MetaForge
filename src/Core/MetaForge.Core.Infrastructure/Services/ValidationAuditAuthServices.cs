...
public class DynamicValidationService : IDynamicValidationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var entity = await GetEntityContext(field.LookupEntity!).FindAsync(lookupEntity, [lookupId], cancellationToken);
        if (entity == null)
            return false;
    ...
    private async Task<bool> ForeignKeyExistsAsync(string lookupEntity, int id, CancellationToken cancellationToken)
    {
        ...
        var entityContext = GetEntityContext(lookupEntity);
        var setMethod = typeof(DbContext).GetMethod(nameof(DbContext.Set), Type.EmptyTypes)!.MakeGenericMethod(entityType);
        var dbSet = setMethod.Invoke(entityContext, null)!;
    ...
    private DbContext GetEntityContext(string entityName) => _contextResolver.ResolveForEntity(entityName);
