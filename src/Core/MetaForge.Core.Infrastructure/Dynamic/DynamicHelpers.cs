...
public sealed class EntityTypeResolver : IEntityTypeResolver
{
    ...
    public EntityTypeResolver(IModuleDbContextResolver contextResolver)
    {
    ...
    {
        var entityType = _contextResolver.GetAllFeatureEntityTypes()
            .FirstOrDefault(t => string.Equals(t.Name, entityName, StringComparison.OrdinalIgnoreCase));

        if (entityType != null)
            return entityType;

        var coreType = _contextResolver.CoreContext.Model.GetEntityTypes()
            .FirstOrDefault(t => string.Equals(t.ClrType.Name, entityName, StringComparison.OrdinalIgnoreCase))
            ?.ClrType;

        return coreType ?? throw new NotFoundException($"Entity '{entityName}' was not found in the model.");
    }

    public IReadOnlyList<Type> GetAllEntityTypes() => _contextResolver.GetAllFeatureEntityTypes();
